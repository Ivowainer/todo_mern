...
const Card = ({ id, name, description, author, priority }) => {
  ...
  return (
    <div className='card flex flex-col w-full xl:w-1/4 h-48 text-gray-700 px-10 md:px-3 pt-3 relative'>
        ...
        <p className='border-b rounded-t-md shadow-card border-default px-5 py-2 text-center'>{name}<span className={`${priority == 'Low' && 'text-orange-400'} ${priority == 'Medium' && 'text-orange-600'} ${priority == 'High' && 'text-red-600'} text-[13px] font-semibold lowercase`}> {priority}</span></p>
